...
PROLONGED_CLOSURE_DURATION = 3  # seconds to trigger alert from TFLite results
...
# ------------------------ Blink Detection (Mediapipe-based from File1) ------------------------
...
# ------------------------ TFLite-Based Eye & Yawn Detector (from File2) ------------------------
class TFLiteDetector:
    def __init__(self):
        ...
        # For measuring prolonged eye closure using timestamps
        self.eye_closed_start = None

        # For consecutive frame counting:
        self.consecutive_closed_frames = 0
        self.CLOSED_FRAMES_REQUIRED = 3
    ...
    def detect_eye_status(self, frame):
        ...
        for (x, y, w, h) in faces:
            ...
            for (ex, ey, ew, eh) in eyes:
                ...
                if prediction > EYE_THRESHOLD:
                    self.consecutive_closed_frames += 1
                else:
                    self.consecutive_closed_frames = 0

                # Optionally, still update the buffer if needed:
                self.eye_state_buffer.append(prediction)

                if self.consecutive_closed_frames >= self.CLOSED_FRAMES_REQUIRED:
                    new_state = "Closed"
...
while cap.isOpened():
    ...
    if results.multi_face_landmarks:
        for face_landmarks in results.multi_face_landmarks:
            ...
            elif current_eye_state == "Closed" and tflite_eye_duration >= PROLONGED_CLOSURE_DURATION:
